...
pub fn ensure_worktree(repo: &Path, branch: &str) -> Result<(PathBuf, bool)> {
    ...
    {
        anyhow::bail!("Branch name may only contain letters, numbers, - _ / + .");
    }
    if !is_repo(repo) {
        anyhow::bail!("Project is not a git repo — spawn without 'branch' to work in the project directory");
    }
    let parent = repo
        .parent()
        .ok_or_else(|| anyhow::anyhow!("Cannot determine repo parent"))?;
    let repo_name = repo
        .file_name()
        .and_then(|n| n.to_str())
    ...
    if path.exists() {
        anyhow::bail!("Worktree path exists but is not a directory: {}", path.display());
    }
    add(repo, &path, Some(branch))?;
    Ok((path, true))
}
...
";
    ...
    fn init_repo(label: &str) -> std::path::PathBuf {
        let root = std::env::temp_dir().join(format!("harness-wt-{label}-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        run_git(&["init", "-b", "main"], &root, 30).unwrap();
        run_git(&["config", "user.email", "t@t"], &root, 30).unwrap();
    ...
    fn ensure_worktree_creates_then_reuses() {
        ...
        assert!(ensure_worktree(&plain, "feature-y").is_err());
        let _ = std::fs::remove_dir_all(&plain);

        remove(&root, &path.to_string_lossy(), true).unwrap();
        let _ = std::fs::remove_dir_all(&root);
    }
}
