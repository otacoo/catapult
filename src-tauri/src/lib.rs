...
#[tauri::command]
async fn set_sound_permissions(enabled: bool, state: State<'_, AppState>) -> Result<(), String> {
    let mut config = state.config.lock().unwrap();
...
#[tauri::command]
async fn set_sound_errors(enabled: bool, state: State<'_, AppState>) -> Result<(), String> {
    let mut config = state.config.lock().unwrap();
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .plugin(tauri_plugin_single_instance::init(|app, _args, _cwd| {
            // A second launch focuses the existing window instead of spawning a copy.
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            harness_api::harness_memory_get,
            harness_api::harness_memory_set,
            harness_api::harness_skills_list,
            ...
            harness_api::get_harness_system_prompt_default,
