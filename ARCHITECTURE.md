...
The Advanced tab covers an extended set of parameters including: MoE CPU offloading (`cpu-moe`, `n-cpu-moe`), weight repacking (`no-repack`), host tensor offload (`no-op-offload`), device bypass (`no-host`), memory auto-fitting (`--fit`, `--fit-target`, `--fit-ctx`), KV unified buffer (`kv-unified`), N-gram speculation (`spec-ngram-size-n/m`, `spec-ngram-min-hits`), lookup cache files, draft model threading/device params, embedding/classification separators, WebUI config overrides, and `reuse-port`. Built-in tools (`--tools`) and MCP servers are configured app-wide on the Tools/MCP page, not per-run.
...
**MCP servers**: The Tools/MCP page also manages MCP (Model Context Protocol) servers persisted to `{data_dir}/catapult/mcp.json` in llama.cpp's Cursor-compatible shape — `{"mcpServers": { "<name>": { "command", "args", "env", "cwd", "timeout_ms" } }}` (see `mcp.rs`; `list_mcp_servers` / `save_mcp_servers` commands). When any servers are configured, `start_server` appends `--mcp-servers-config <path>` so llama-server spawns each stdio child and exposes its tools as `<server>_<tool>` in the WebUI. Entries without a command are dropped, matching llama.cpp's parsing. llama.cpp has no built-in web fetch/search tool — those arrive via an MCP server. **Windows shim wrapping**: llama.cpp spawns MCP children with `CreateProcess`, which cannot launch `.cmd`/`.bat` scripts — so package-manager shims like `npx` fail with `failed to spawn`. `runtime_mcp_config_path` detects commands that resolve to a `.cmd`/`.bat` (mirroring `SearchPathW` over `PATHEXT`+`PATH`) and materializes a runtime copy at `mcp_effective.json` with such commands rewritten as `cmd /c <command> <args>`; the persisted `mcp.json` stays portable. Resolution is deliberately conservative — explicit `.exe`/`.com` paths or found executables are left alone, anything else (including bare names that resolve to nothing on PATH) gets wrapped, so an environment/PATH quirk in the app process can't silently bypass the fix. `effective_config`/`needs_cmd_wrapper`/`apply_shim_wrap` are unit-tested cross-platform. **Beware the second MCP path**: llama.cpp's WebUI also has a *browser-side* MCP client (Settings → MCP Client) that connects to MCP servers from the browser itself; it requires the MCP CORS proxy (`--webui-mcp-proxy`, the "WebUI MCP Proxy" toggle on the Server page, reflected in `/props.cors_proxy_enabled`) for cross-origin endpoints. That is independent of Catapult's server-side stdio spawning — MCP tools that surface in the chat picker as `<server>_<tool>` come from `--mcp-servers-config`; browser-side servers configured per-origin in localStorage are managed entirely by the WebUI. When llama-server is started with no args it runs in **router mode** (a different model-router server, `/props.role = "router"`), whose UI differs; Catapult always runs the classic single-model server.
...
- **Rust:** `cargo test` — 140 unit tests in `#[cfg(test)]` modules covering asset scoring, backend detection, CLI arg building, tool sanitization, MCP config round-tripping and Windows shim wrapping, quant extraction, size estimation, filename parsing, GGUF parsing, hardware config suggestions, split file parsing, imatrix detection, split model consolidation, `presets.ini` parsing, `apply_hf_preset_params`, preset name derivation, `AppConfig.model_presets` round-tripping, release-tag parsing, and config round-trips.
